...
# get_distractors(keyword, " ".join(keyword_sentences), SentenceTransformer('msmarco-distilbert-base-v3'), 40, 0.2)
...
def get_keywords(originaltext):
    # Assuming you have a function named get_nouns_multipartite
...
def generate_FITB_questions(context, num_questions):
    ...
    for keyword, keyword_sentences in keyword_sentence_mapping.items():
        if keyword.lower() in used_keywords:
            continue
        used_keywords.add(keyword.lower())
        distractors = get_distractors(keyword, " ".join(keyword_sentences), SentenceTransformer('msmarco-distilbert-base-v3'), 40, 0.2)
        if not distractors:
            continue
...
def generate_fill_in_the_blanks(context, num_questions):
    generated_questions = generate_FITB_questions(context, num_questions)
    output = ""
...
def get_word_definition(word):
    ...
    Returns:
    - list of str: A list of definitions for the word. If no definitions are found, returns an empty list.
    """
    word = word.lower()
    synsets = wn.synsets(word)
    definitions = []
    ...
    if definitions:
       ...
       for definition in definitions:
        print("the definition of the given word is: ",definition)
...
@app.route('/get_antonyms', methods=['POST'])
def get_antonyms_route():
    word = request.form['word']
    antonyms = get_antonyms(word)
    synonyms = None  # Initialize synonyms variable

    if not antonyms:
        synonyms = get_synonyms(word)
        antonyms = []
        for synonym in synonyms:
            antonyms.extend(get_antonyms(synonym))

    if antonyms:
        response = f"Original word: {word.capitalize()}\n"
        if synonyms:
            response += f"Synonyms: {', '.join(synonyms)}\n"
        response += f"Antonyms: {', '.join(antonyms)}"
    else:
        response = f"No antonyms found for '{word}' and its synonyms."
...
def get_synonyms_route():
    ...
    if synonyms:
        response = f"Synonyms for '{word}': {', '.join(synonyms)}"
    else:
        response = f"No synonyms found for '{word}'."
...
@app.route('/generate_fill_in_the_blanks', methods=['POST'])
def generate_fill_in_the_blanks_route():
    context = request.form['context']
    num_questions = int(request.form['num_questions'])
    generated_questions = generate_fill_in_the_blanks(context, num_questions)
    return generated_questions
